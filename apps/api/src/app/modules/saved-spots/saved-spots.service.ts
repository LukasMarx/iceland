...
export class SavedSpotsService {
  ...
  async getSavedSpots(tripId: string, query: { tripId?: string; limit?: string; cursor?: string }, baseUrl?: string) {
    const resolvedTripId = query.tripId ?? tripId;
    const limit = Math.min(Number(query.limit ?? 20), 50);

    const trip = await this.prisma.trip.findFirst({ where: { id: resolvedTripId } });
  ...
  async saveSpot(tripId: string, body: { spotId: string; tripId?: string }, baseUrl?: string) {
    ...
    await this.prisma.savedSpot.upsert({
      where: { tripId_spotId: { tripId: resolvedTripId, spotId: body.spotId } },
      create: { tripId: resolvedTripId, spotId: body.spotId },
      update: {},
    });

    const allSaved = await this.prisma.savedSpot.findMany({ where: { tripId: resolvedTripId }, select: { spotId: true } });
  ...
  async unsaveSpot(tripId: string, spotId: string, query: { tripId?: string }) {
    const resolvedTripId = query.tripId ?? tripId;
    ...
    await this.prisma.savedSpot.deleteMany({ where: { tripId: resolvedTripId, spotId } });
