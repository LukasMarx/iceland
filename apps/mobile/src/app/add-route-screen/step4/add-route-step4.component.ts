...
export class AddRouteStep4Component {
  ...
  protected readonly wizardMarkers = computed((): MapMarker[] => {
    ...
    if (base) markers.push({ id: 'start', coordinates: base.location, color: '#101114', size: 'lg', label: base.name });
    for (const spot of this.candidateStops()) {
      markers.push({ id: spot.id, coordinates: spot.location, color: statusColor(spot.status.status), size: this.service.selectedStopIds().includes(spot.id) ? 'lg' : 'md', label: spot.name });
    }
    if (destination) markers.push({ id: 'destination', coordinates: destination, color: '#2563eb', size: 'lg', label: hotel?.name ?? base?.name ?? 'Destination' });
    return markers;
  });
  ...
  protected readonly routeLines = computed((): MapRoute[] => {
    ...
    return [{
      ...
      coordinates: [base.location, ...this.selectedStops().map((spot: Spot) => spot.location), destination],
      color: '#101114', width: 4, opacity: 0.72,
    }];
  });
  ...
  protected useRecommended(): void { this.service.setSelectedStops(this.recommendedStopIds()); this.continue(); }
  protected chooseManually(): void { this.manualMode.set(true); if (this.service.selectedStopIds().length === 0) this.service.setSelectedStops(this.recommendedStopIds().slice(0, 2)); }
  protected driveDirectly(): void { this.service.setSelectedStops([]); this.continue(); }
  protected toggleStop(spot: Spot): void { this.service.toggleStop(spot.id); }
  protected onMarkerClick(markerId: string): void {
    const spot = this.candidateStops().find((candidate: Spot) => candidate.id === markerId);
    if (spot) { this.toggleStop(spot); this.manualMode.set(true); }
  }
  protected continue(): void { this.service.step.set(5); void this.router.navigateByUrl('/routes/add/step5'); }
  ...
  protected attractionImage(spot: Spot): string { return spotBackground(spot); }
  protected isSelected(spot: Spot): boolean { return this.service.selectedStopIds().includes(spot.id); }
}
